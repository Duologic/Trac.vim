...
# FIXME Global variables; THESE SHOULD GO ASAP!
tr_server = None
browser = None
mode = 0
...
class VimWindow:
    ...
    def isprepared(self):
        ...
        if self.buffer is None or \
                len(dir(self.buffer)) == 0 or self.getwinnr() == -1:
            return 0
        return 1
    ...
    def write(self, msg):
        ...
        if self.firstwrite == 1:
            ...
            #TODO tickets #7 and #56 setting to utf-8 causes sporadic
            #     ticket Encoding errors
...
class TracWiki(TracRPC):
    ...
    def getPage(self, name, b_create=False, revision=None):
        ...
        try:
            ...
            if b_create:
                try:
                    self.server.wiki.putPage(name, wikitext, {
                ...
                print "Could not find page " + name + \
                      ". Use :TWCreate " + name + " to create it"
                #TODO this will create the page anyway. possible bug if
    ...
    def save(self, comment):
        ...
        if comment == '':
            comment = tr_server.default_comment
        self.server.wiki.putPage(self.currentPage,
                                 tr_server.uiwiki.wikiwindow.dump(),
                                 {"comment": comment})
    ...
    def get_page_info(self):
        ...
        return 'Page: ' + info['name'] + ' Revision: ' + \
               str(info['version']) + ' Author: ' + info['author']
    ...
    def addAttachment(self, file):
        ...
        self.server.wiki.putAttachment(self.currentPage + '/' + file_name,
    ...
    def listAttachments(self):
        ...
        self.current_attachments = \
            self.server.wiki.listAttachments(self.currentPage)
    ...
    def html_view(self, page):
        ...
        html = '<html><body>' + self.server.wiki.getPageHTML(page) + \
               '</body></html>'
    ...
    def vim_diff(self, revision=None):
        ...
        tr_server.uiwiki.tocwindow.resize_width(30)
        tr_server.uiwiki.wikiwindow.set_focus()
        #tr_server.uiwiki.wikiwindow.resize_width(
        #   int(tr_server.uiwiki.wikiwindow.width / 2))
...
class WikiTOContentsWindow(NonEditableWindow):
    ...
    def on_create(self):
        vim.command('nnoremap <buffer> <cr> ' +
                    ':python trac.wiki_view("CURRENTLINE")<cr>')
        vim.command('nnoremap <buffer> <2-LeftMouse> ' +
                    ':python trac.wiki_view("CURRENTLINE")<cr>')
        vim.command('nnoremap <buffer> <Space> ' +
                    ':python trac.html_view()<cr><cr><cr>')
        vim.command('nnoremap <buffer> :q<cr> ' +
                    ':python trac.normal_view()<cr>')
...
class TracTicket(TracRPC):
    ...
    def getAllTickets(self, owner, b_use_cache=False):
        ...
        ticket_list = \
            "(Hit <enter> or <space> on a line containing Ticket:>>)\n"
        ...
        for ticket in tickets:
            ...
            if ticket[3]["status"] != "closed" and self.filter.check(ticket):
                ...
                str_ticket += "\n    ".join(
                    ticket[3]["description"].strip().split("\n")) + "\n"
            ...
            if change[4] != '':
                ...
                if change[2] == 'comment':
                    str_ticket += '== ' + my_time + \
                        " (" + change[1] + ": comment) ==\n    "
                    str_ticket += "\n    ".join(
                        change[4].strip().split("\n")) + "\n"
                elif brief == 0:
                    if change[2] == 'description':
                        str_ticket += '== ' + my_time + \
                            " (" + change[1] + ": modified description) ==\n"
                    else:
                        str_ticket += '== ' + my_time + " (" + change[1] + \
                            " set " + change[2] + " to " + change[4] + ") ==\n"

        return str_ticket

    def updateTicket(self, comment, attribs={}, notify=False):
        """ add ticket comments change attributes """
        return self.server.ticket.update(self.current_ticket_id,
                                         comment, attribs, notify)

    def createTicket(self, description, summary, attributes={}):
        """ create a trac ticket """
        self.current_ticket_id = self.server.ticket.create(
            summary, description, attributes, False)

    def addAttachment(self, file):
        ''' Add attachment '''
        #file_name = os.path.basename(file)
        self.server.ticket.putAttachment(self.current_ticket_id,
                                         file, 'attachment',
                                         xmlrpclib.Binary(open(file).read()))
    ...
    def set_attr(self, option, value):
        ...
        if tr_server.uiticket.mode == 0 or \
                not tr_server.ticket.current_ticket_id:
            print "Cannot make changes when there is no current ticket " + \
                "open in Ticket View"
            return 0

        comment = tr_server.uiticket.commentwindow.dump()
        tr_server.uiticket.commentwindow.clean()
        ...
        tr_server.ticket.updateTicket(comment, attribs, False)
        tr_server.ticket_view(tr_server.ticket.current_ticket_id, True)
    ...
    def add_comment(self):
        ...
        tr_server.ticket.updateTicket(comment, attribs, False)
        tr_server.uiticket.commentwindow.clean()
        tr_server.ticket_view(tr_server.ticket.current_ticket_id)
    ...
    def update_description(self):
        ...
        if tr_server.uiticket.mode == 0 or \
                not tr_server.ticket.current_ticket_id:
            print "Cannot make changes when there is no current ticket is " + \
                "open in Ticket View"
            return 0
        ...
        tr_server.ticket.updateTicket('', attribs, False)
        tr_server.uiticket.commentwindow.clean()
        tr_server.ticket_view(tr_server.ticket.current_ticket_id)
    ...
    def create(self, summary='new ticket', type=False, server=False):
        ...
        if not server:
            tr_server.set_current_server(server, True, 'ticket')
            description = ''
        else:
            description = tr_server.uiticket.commentwindow.dump()

        if tr_server.uiticket.mode == 0 and not server:
            print "Can't create a ticket when not in Ticket View"
            return 0

        confirm = vim.eval('confirm("Create Ticket on ' +
                           tr_server.server_name + '?", "&Yes\n&No\n",2)')
        ...
        tr_server.ticket.createTicket(description, summary, attribs)
        tr_server.uiticket.commentwindow.clean()
        tr_server.ticket_view(tr_server.ticket.current_ticket_id)
    ...
    def session_save(self):
        ...
        serverdir = re.sub(r'[^\w]', '', tr_server.server_name)

        if os.path.isdir(directory + '/' + (not serverdir)):
    ...
    def session_load(self):
        ...
        vim.command("bdelete TICKET_WINDOW")
        vim.command("bdelete TICKET_COMMENT_WINDOW")
        tr_server.ticket_view(self.current_ticket_id)

    def session_component_save(self, component=False):
        """Save a session based on the component supplied or the
        current ticket """
        global tr_server

        if not component:
            if not self.current_component:
                print "You need to have an active ticket or " + \
    ...
    def session_component_load(self, component):
        """ Loads a session based on the component supplied or the
        current ticket """
        global tr_server
    ...
    def context_set(self):
        ...
        if (re.match("Milestone:", line) is not None):
            self.get_options(0)
        elif (re.match("Type:", line) is not None):
            self.get_options(1)
        elif (re.match("Status:", line) is not None):
            self.get_options(2)
        elif (re.match("Resolution:", line) is not None):
            self.get_options(3)
        elif (re.match("Priority:", line) is not None):
            self.get_options(4)
        elif (re.match("Severity:", line) is not None):
            self.get_options(5)
        elif (re.match("Component:", line) is not None):
            self.get_options(6)
        else:
    ...
    def summary_view(self):
        global tr_server
        tr_server.uiticket.summarywindow.create('belowright 10 new')
        tr_server.uiticket.summarywindow.write(
            self.getAllTicketsSummary(tr_server.user, False))
        tr_server.uiticket.mode = 2
...
class TracTicketSort:
    ...
    def sort(self, tickets):
        ...
        sorted_tickets = []
        #for milestone in tr_server.ticket.a_option[7]:
        for milestone in tr_server.ticket.a_option[0]:
            for ticket in tickets:
...
class TracTicketFilter:
    ...
    def add(self, keyword, attribute,
            b_whitelist=True, b_refresh_ticket=True):
...
class TicketSummaryWindow(VimWindow):
    ...
    def on_write(self):
        try:
            ...
            vim.command('echo you should get the Align Plugin ' +
                        'to make this view work best')
...
class TicketCommentWindow (VimWindow):
    ...
    def on_create(self):
        vim.command('nnoremap <buffer> :w<cr> ' +
                    ':python trac.ticket.add_comment()<cr>')
        vim.command('nnoremap <buffer> :wq<cr> ' +
                    ':python trac.ticket.add_comment()<cr>' +
                    ':python trac.normal_view()<cr>')
        vim.command('nnoremap <buffer> :q<cr> :python trac.normal_view()<cr>')
...
class TracTimeline:
    def read_timeline(self):
        ...
        query_string = \
            'timeline?ticket=on&changeset=on&wiki=on&' + \
            'max=50&daysback=90&format=rss'
        feed = tr_server.wiki.server_url.replace('login/xmlrpc', query_string)
        ...
        for item in d['items']:
            ...
            str_feed += "Update: " + \
                strftime("%Y-%m-%d %H:%M:%S", item.updated_parsed) + "\n"
...
class Trac:
    ...
    def ticket_view(self, id=False, b_use_cache=False):
        ...
        if style == 'summary':
            ...
            self.uiticket.summarywindow.write(
                self.ticket.getAllTicketsSummary(self.user, b_use_cache))
        else:
            self.uiticket.tocwindow.clean()
            self.uiticket.tocwindow.write(
                self.ticket.getAllTickets(self.user, b_use_cache))
        ...
        if not (id):
            if not self.ticket.current_ticket_id:
                ...
                self.uiticket.ticketwindow.write(
                    self.ticket.getTicket(tr_server.ticket.current_ticket_id))
    ...
    def set_current_server(self, server_key, quiet=False, view=False):
        ...
        if not quiet:
            ...
            {'wiki': self.wiki_view,
             ...
             'timeline': self.timeline_view}[view]()
    ...
    def normal_view(self):
        tr_server.uiserver.normal_mode()
        tr_server.uiwiki.normal_mode()
        tr_server.uiticket.normal_mode()
        tr_server.uisearch.normal_mode()
        tr_server.uitimeline.normal_mode()
    ...
    def add_attachment(self, file):
        ...
        if self.uiwiki.mode == 1:
            ...
            print "Adding attachment to ticket #" + \
                str(self.ticket.current_ticket_id) + '...'
    ...
    def preview(self, b_dump=False):
        ...
        if self.uiwiki.mode == 1:
            print "Retrieving preview from wiki " + \
                self.wiki.currentPage + '...'
        ...
        html = '<html><body>' + self.wiki.getWikiHtml(wikitext) + \
            '</body></html>'
    ...
    def changeset_view(self, changeset, b_full_path=False):
        #if b_full_path:
        changeset = self.wiki.server_url.replace('login/xmlrpc',
                                                 'changeset/' + changeset)
...
def trac_init():
    ...
    tr_server = Trac(comment, server_list)
